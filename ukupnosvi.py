...
# Filtriranje podataka za odabrani datumski raspon
filtered_df = df[(df['datumvreme'].dt.date >= pd.to_datetime(pocetni_datum).date()) & (df['datumvreme'].dt.date <= pd.to_datetime(krajnji_datum).date())].copy()
...
plt.title(f'Prisutni u Gimnaziji od {pocetni_datum} do {krajnji_datum}')
plt.xlabel('Vreme')
plt.ylabel('Broj prisutnih')
